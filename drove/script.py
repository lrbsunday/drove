...
class CliScript(object):
    ...
    def _exit_handler(self, *args, **kwargs):
    ...
    def __call__(self):
        ...
        self.args = args = cmdopt.parse_args()
        ...
        if args.environment:
            for config_val in args.environment:
        ...
        self.plugins = plugins = drove.plugin.PluginManager(config, channel)
        ...
        daemon = drove.daemon.Daemon.create(
            self._daemon, self._exit_handler)  # handle TERM
        if args.foreground:
